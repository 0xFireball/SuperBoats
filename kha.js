	...
	,step: function() {
		...
		if(targetSetpoint != null) {
			...
			if(Math.abs(facingAngle - angleToSetpoint) > Math.PI / 8) {
				...
				up = true;
			} else if(this.style == ai_Style.Defensive && Math.sqrt(nv.x * nv.x + nv.y * nv.y) > chaseRadius * 1.33333333333333326) {
				up = true;
			}
		}
...
var kha_FontList = function() {
	this.names = ["champagneLimousines"];
	this.champagneLimousinesDescription = { files : ["champagneLimousines.ttf"], type : "font", name : "champagneLimousines"};
	this.champagneLimousinesName = "champagneLimousines";
	this.champagneLimousines = null;
};
$hxClasses["kha.FontList"] = kha_FontList;
kha_FontList.__name__ = true;
kha_FontList.prototype = {
	champagneLimousines: null
	,champagneLimousinesName: null
	,champagneLimousinesDescription: null
	,champagneLimousinesLoad: function(done) {
		kha_Assets.loadFont("champagneLimousines",function(font) {
			done();
		});
	}
	,champagneLimousinesUnload: function() {
		this.champagneLimousines.unload();
		this.champagneLimousines = null;
	}
	,names: null
...
var n4_NCamera = function(X,Y,Width,Height,Zoom) {
	if(Zoom == null) {
		Zoom = 0;
	}
	if(Height == null) {
	...
	this.x = X;
	this.y = Y;
	this.width = Width <= 0 ? n4_NGame.width : Width;
	this.height = Height <= 0 ? n4_NGame.height : Height;
	this.zoom = Zoom == 0 ? n4_NCamera.defaultZoom : Zoom;
	this.initialZoom = this.zoom;
};
$hxClasses["n4.NCamera"] = n4_NCamera;
n4_NCamera.__name__ = true;
n4_NCamera.prototype = {
	x: null
	,y: null
	,width: null
	,height: null
	...
	,initialZoom: null
	,angle: null
	,render: function(f,drawRoot) {
		f.get_g2().begin(false);
		f.get_g2().pushTranslation(this.x,this.y);
		f.get_g2().pushRotation(this.angle,n4_NGame.width / 2,n4_NGame.height / 2);
		drawRoot.render(f);
		f.get_g2().end();
	}
	,__class__: n4_NCamera
};
...
n4_NEntity.prototype = $extend(n4_NBasic.prototype,{
	...
	,get_damage: function() {
		return 1 - (this.health < 0 ? 0 : this.health) / this.maxHealth;
	}
...
var n4_NG = function() { };
$hxClasses["n4.NG"] = n4_NG;
n4_NG.__name__ = true;
n4_NG.collide = function(ObjectOrGroup1,ObjectOrGroup2,NotifyCallback) {
	return n4_NG.overlap(ObjectOrGroup1,ObjectOrGroup2,NotifyCallback,n4_NEntity.separate);
};
n4_NG.overlap = function(ObjectOrGroup1,ObjectOrGroup2,NotifyCallback,ProcessCallback) {
	...
	n4_system_NQuadTree.divisions = n4_NG.worldDivisions;
	var quadTree = n4_system_NQuadTree.recycle(n4_NG.worldBounds.x,n4_NG.worldBounds.y,n4_NG.worldBounds.width,n4_NG.worldBounds.height);
	quadTree.load(ObjectOrGroup1,ObjectOrGroup2,NotifyCallback,ProcessCallback);
	var result = quadTree.execute();
	quadTree.destroy();
	return result;
};
n4_NG.get_hypot = function() {
	return Math.sqrt(n4_NGame.width * n4_NGame.width + n4_NGame.height * n4_NGame.height);
};
n4_NG.get_camera = function() {
	return n4_NG.cameras[0];
};
...
n4_NGame.switchState = function(state) {
	...
	if(!n4_NGame.currentState.created) {
...
n4_NGame.ge_update = function() {
	...
	var gdt = n4_NGame._clock.dt;
	if(n4_NGame.stabilizeUpdateInterval) {
		gdt = Math.min(gdt,2 / n4_NGame.targetFramerate);
	}
...
n4_NGame.ge_render = function(framebuffer) {
	...
	var renderCall = function(f) {
		f.get_g2().begin(true,n4_NGame.currentState.bgColor);
		n4_NG.get_camera().render(f,n4_NGame.currentState);
		f.get_g2().end();
	};
	if(n4_NGame.useDoubleBuffering) {
		renderCall(n4_NGame._backbuffer);
...
n4_NGame.onInitialized = function() {
	...
	n4_NG.keys = new n4_input_keyboard_NKeyboard();
	n4_NG.touches = new n4_input_touch_NTouchSurface();
	...
	n4_NG.cameras.push(new n4_NCamera(0,0,n4_NGame.width,n4_NGame.height,1.0));
...
n4_entities_NSprite.prototype = $extend(n4_NEntity.prototype,{
	...
	,render: function(f) {
		...
		} else if(this.graphicRenderer != null) {
			this.graphicRenderer(f);
		}
	...
	,makeGraphic: function(Width,Height,GraphicColor) {
		var _gthis = this;
		this.graphicRenderer = function(f) {
			f.get_g2().begin(false);
			f.get_g2().set_color(GraphicColor);
			f.get_g2().fillRect(_gthis.x,_gthis.y,Width,Height);
			f.get_g2().end();
		};
		this.set_width(Width);
		this.set_height(Height);
	...
	,loadGraphic: function(asset) {
		var _gthis = this;
		kha_Assets.loadImageFromPath(asset,true,function(i) {
			_gthis.set_graphic(i);
			_gthis.set_width(i.get_width());
			_gthis.set_height(i.get_height());
		});
	}
...
var sprites_Boat = function(X,Y) {
	...
	n4_entities_NSprite.call(this,X,Y);
	this.maxHealth = this.health = 100000;
...
sprites_Boat.prototype = $extend(n4_entities_NSprite.prototype,{
	...
	,manageHealth: function() {
		if(this.get_damage() > 0.2 && this.get_damage() <= 0.5) {
			var _g1 = 0;
			var _g = 4 * this.get_damage() | 0;
			while(_g1 < _g) {
				++_g1;
				Registry.PS.explosionEmitter.emitSquare(this.get_center().x,this.get_center().y,Math.random() * 6 + 3 | 0,n4_effects_particles_NParticleEmitter.velocitySpread(90),n4_util_NColorUtil.randCol(0.3,0.3,0.3,0.05),1.8);
			}
		} else if(this.get_damage() > 0.5 && this.get_damage() <= 0.7) {
			var _g11 = 0;
			var _g2 = 8 * this.get_damage() | 0;
			while(_g11 < _g2) {
				++_g11;
				Registry.PS.explosionEmitter.emitSquare(this.get_center().x,this.get_center().y,Math.random() * 6 + 3 | 0,n4_effects_particles_NParticleEmitter.velocitySpread(90),n4_util_NColorUtil.randCol(0.8,0.5,0.2,0.2),1.8);
			}
		} else if(this.get_damage() > 0.7) {
			var _g12 = 0;
			var _g3 = 12 * this.get_damage() | 0;
			while(_g12 < _g3) {
				++_g12;
				Registry.PS.explosionEmitter.emitSquare(this.get_center().x,this.get_center().y,Math.random() * 6 + 3 | 0,n4_effects_particles_NParticleEmitter.velocitySpread(90),n4_util_NColorUtil.randCol(0.9,0.3,0.2,0.1),2.2);
			}
		}
		if(this.get_damage() >= 1) {
			this.explode();
			this.destroy();
		}
	}
	,drawSpray: function() {
		...
		var _g1 = 0;
		var _g = this.sprayAmount;
		while(_g1 < _g) {
			++_g1;
			Registry.PS.lowerEmitter.emitSquare(this.get_center().x,this.get_center().y,(Math.random() * 10 | 0) + 1,n4_effects_particles_NParticleEmitter.velocitySpread(this.spraySpread,particleTrailVector.x,particleTrailVector.y),n4_util_NColorUtil.randCol(0.2,0.6,0.8,0.2),Math.random());
		}
	}
	,explode: function() {
		var _g = 0;
		while(_g < 50) {
			++_g;
			Registry.PS.explosionEmitter.emitSquare(this.get_center().x,this.get_center().y,(Math.random() * 10 | 0) + 1,n4_effects_particles_NParticleEmitter.velocitySpread(this.spraySpread,this.velocity.x / 4,this.velocity.y / 4),n4_util_NColorUtil.randCol(0.95,0.95,0.1,0.05),Math.random());
		}
	}
...
var sprites_DummyBoat = function(X,Y) {
	...
	n4_entities_NSprite.call(this,X,Y);
	this.makeGraphic(20,48,n4_util_NColorUtil.randCol(0.3,0.3,0.7,0.3));
	this.maxVelocity.set(200,200);
	this.maxAngular = Math.PI * 0.1;
	this.randomizeMotion();
};
$hxClasses["sprites.DummyBoat"] = sprites_DummyBoat;
sprites_DummyBoat.__name__ = true;
sprites_DummyBoat.__super__ = n4_entities_NSprite;
sprites_DummyBoat.prototype = $extend(n4_entities_NSprite.prototype,{
	update: function(dt) {
		var particleTrailVector = this.velocity.toVector();
		particleTrailVector.rotate(new n4_math_NPoint(0,0),180);
		particleTrailVector.scale(0.7);
		...
		while(_g < 5) {
			++_g;
			Registry.MS.emitter.emitSquare(this.get_center().x,this.get_center().y,(Math.random() * 10 | 0) + 1,n4_effects_particles_NParticleEmitter.velocitySpread(40,particleTrailVector.x,particleTrailVector.y),n4_util_NColorUtil.randCol(0.2,0.6,0.8,0.2),Math.random());
		}
		this.keepInBounds();
		n4_entities_NSprite.prototype.update.call(this,dt);
	}
	,randomizeMotion: function() {
		this.acceleration.set_y(Math.random() * -90);
...
sprites_GreenBoat.prototype = $extend(sprites_Boat.prototype,{
	...
	,acquireTarget: function() {
		var _gthis = this;
		var target = null;
		var minDistance = n4_NG.get_hypot() * 2;
...
sprites_Warship.prototype = $extend(sprites_Boat.prototype,{
	...
	,update: function(dt) {
		this.movement();
		this.attackTimer += dt;
		if(this.attackTimer > sprites_Warship.attackTime) {
			this.attackPlayer();
			++this.attackCount;
			this.attackTimer = 0;
		}
	...
	,attackPlayer: function() {
		...
		if(this.attackCount % 3 == 0) {
			...
			var pVelVec = new n4_math_NPoint(vx,vy);
			pVelVec.rotate(new n4_math_NPoint(0,0),Math.random() * ((Math.random() * this.cannonMissRange * 2 - this.cannonMissRange) * (180 / Math.PI)));
			vx = pVelVec.x;
			vy = pVelVec.y;
			this.shootProjectile(projectile,vx,vy);
		}
		if(this.attackCount % 7 == 0) {
			...
			vx1 = pVelVec1.x;
			vy1 = pVelVec1.y;
			this.shootProjectile(projectile1,vx1,vy1);
		}
	}
	,shootProjectile: function(pj,vx,vy) {
		pj.velocity.set(vx,vy);
		Registry.PS.projectiles.add(pj);
		var _this = pj.get_momentum().scale(1 / this.mass);
		_this.set_x(_this.x * -1);
		_this.set_y(_this.y * -1);
		this.velocity.addPoint(_this);
		if(this.x > n4_NGame.width) {
			this.set_x(this.x % n4_NGame.width);
		}
		if(this.y > n4_NGame.height) {
			this.set_y(this.y % n4_NGame.height);
		}
		if(this.x < 0) {
			var _g = this;
			_g.set_x(_g.x + n4_NGame.width);
		...
		while(_g2 < 14) {
			++_g2;
			Registry.PS.smokeEffectEmitter.emitSquare(this.x,this.y,6,n4_effects_particles_NParticleEmitter.velocitySpread(45,vx / 4,vy / 4),n4_util_NColorUtil.randCol(0.5,0.5,0.5),0.8);
		}
	}
	,movement: function() {
		var target = this.acquireTarget();
...
var sprites_Minion = function(X,Y) {
	...
	this.sprayAmount = 20;
	this.spraySpread = 80;
	this.angularThrust = 0.034 * Math.PI;
	this.maxAngular = Math.PI / 4;
	this.maxVelocity.set(135,135);
...
var sprites_Mothership = function(X,Y) {
	...
	this.sprayAmount = 20;
	this.spraySpread = 80;
	this.angularThrust = 0.03 * Math.PI;
	this.maxAngular = Math.PI / 5;
	this.maxVelocity.set(60,60);
	if(Registry.levelNum > 0) {
		var lvl = Registry.levelNum;
		this.health *= 2 * Math.pow(1.1,lvl);
		this.minionDelay *= 1 / Math.pow(4,lvl);
		this.maxMinionCount += Math.ceil(Math.pow(1.6,lvl));
		this.minionSpawnChance = this.minionSpawnChance / (lvl + 1) | 0;
		this.maxHealth = this.health;
		this.hullShieldMax = this.hullShieldIntegrity = 180000 * Math.pow(1.2,lvl);
		this.hullShieldRegen = 2 + (lvl - 1) * 2;
	}
...
sprites_Mothership.prototype = $extend(sprites_Warship.prototype,{
	...
	,minionDelay: null
	,update: function(dt) {
		if(this.get_damage() > this.minionDelay && this.minionCount < this.maxMinionCount && (Math.random() * this.minionSpawnChance | 0) == 4) {
			this.minionCount++;
			var minionDist = n4_NG.get_hypot() / 4;
...
sprites_PlayerBoat.prototype = $extend(sprites_GreenBoat.prototype,{
	...
	,movement: function() {
		...
		var right = false;
		var down = false;
		left = n4_NG.keys.pressed(["J","LEFT"]);
		up = n4_NG.keys.pressed(["I","UP"]);
		right = n4_NG.keys.pressed(["L","RIGHT"]);
		down = n4_NG.keys.pressed(["K","DOWN"]);
		this.moveDefault(up,left,right,down);
		this.attacking = n4_NG.keys.pressed(["F","M"]);
	}
...
var sprites_projectiles_Projectile = function(X,Y) {
	...
	this.damageScale = 0.0416666666666666644;
	this.damageFactor = 10;
	this.movementSpeed = 100;
	n4_entities_NSprite.call(this,X,Y);
	this.mass = 500;
};
$hxClasses["sprites.projectiles.Projectile"] = sprites_projectiles_Projectile;
sprites_projectiles_Projectile.__name__ = true;
...
sprites_projectiles_Projectile.prototype = $extend(n4_entities_NSprite.prototype,{
	...
	,damageScale: null
	,explode: function() {
		var transferredMomentum = this.velocity.toVector().scale(this.mass / this.target.mass);
		this.target.velocity.addPoint(transferredMomentum);
		this.destroy();
	}
	,calculateDamage: function() {
		var _this = this.get_momentum();
		return this.damageFactor * Math.sqrt(_this.x * _this.x + _this.y * _this.y) * this.damageScale;
	}
	,hitTarget: function() {
	...
	,checkBounds: function() {
		if(this.x < this.get_width() * 2 || this.y < this.get_height() * 2 || this.x > n4_NGame.width + this.get_width() * 2 || this.y > n4_NGame.height + this.get_height() * 2) {
			this.destroy();
		}
	}
...
$hxClasses["sprites.projectiles.Cannonball"] = sprites_projectiles_Cannonball;
sprites_projectiles_Cannonball.__name__ = true;
sprites_projectiles_Cannonball.__super__ = sprites_projectiles_Projectile;
sprites_projectiles_Cannonball.prototype = $extend(sprites_projectiles_Projectile.prototype,{
	update: function(dt) {
		var particleTrailVector = this.velocity.toVector();
		particleTrailVector.rotate(new n4_math_NPoint(0,0),180);
		particleTrailVector.scale(0.7);
		...
		while(_g < 2) {
			++_g;
			Registry.get_currentEmitterState().emitter.emitSquare(this.get_center().x,this.get_center().y,(Math.random() * 6 | 0) + 1,n4_effects_particles_NParticleEmitter.velocitySpread(40,particleTrailVector.x,particleTrailVector.y),n4_util_NColorUtil.randCol(0.5,0.1,0.1,0.1),0.7);
		}
	...
	,explode: function() {
		...
		while(_g < 15) {
			++_g;
			Registry.PS.explosionEmitter.emitSquare(this.get_center().x,this.get_center().y,Math.random() * 8 + 4 | 0,n4_effects_particles_NParticleEmitter.velocitySpread(50),n4_util_NColorUtil.randCol(0.8,0.2,0.2,0.2),1.8);
		}
		var _g1 = 0;
		while(_g1 < 10) {
			++_g1;
			Registry.PS.explosionEmitter.emitSquare(this.get_center().x,this.get_center().y,Math.random() * 8 + 4 | 0,n4_effects_particles_NParticleEmitter.velocitySpread(50),n4_util_NColorUtil.randCol(0.8,0.8,0.2,0.2),1.8);
		}
...
sprites_projectiles_Talon.prototype = $extend(sprites_projectiles_Projectile.prototype,{
	...
	,update: function(dt) {
		...
		while(_g < 2) {
			++_g;
			Registry.get_currentEmitterState().emitter.emitSquare(this.get_center().x,this.get_center().y,(Math.random() * 6 | 0) + 1,n4_effects_particles_NParticleEmitter.velocitySpread(40,particleTrailVector.x,particleTrailVector.y),n4_util_NColorUtil.randCol(0.1,0.9,0.9,0.1),0.7);
		}
...
$hxClasses["sprites.projectiles.Torpedo"] = sprites_projectiles_Torpedo;
sprites_projectiles_Torpedo.__name__ = true;
sprites_projectiles_Torpedo.__super__ = sprites_projectiles_Projectile;
sprites_projectiles_Torpedo.prototype = $extend(sprites_projectiles_Projectile.prototype,{
	...
	,__class__: sprites_projectiles_Torpedo
});
var states_GameOverState = function() {
	n4_NState.call(this);
};
...
states_GameOverState.prototype = $extend(n4_NState.prototype,{
	...
	,create: function() {
		...
		var pbt = new n4e_ui_NEText(0,n4_NGame.height * 0.3,"game over (level " + Registry.levelNum + ")",45);
		pbt.screenCenter(n4_util_NAxes.X);
		this.add(pbt);
		var tt2 = new n4e_ui_NEText(0,n4_NGame.height * 0.65,"mothership health: " + ((1 - Registry.lastMsDamage) * 100 | 0) + "%",32);
...
var states_HelpState = function(Transient,NextState) {
	if(Transient == null) {
		Transient = false;
	}
	n4_NState.call(this);
...
$hxClasses["states.HelpState"] = states_HelpState;
states_HelpState.__name__ = true;
states_HelpState.__super__ = n4_NState;
states_HelpState.prototype = $extend(n4_NState.prototype,{
	emitter: null
	,'transient': null
	,nextState: null
	,create: function() {
		...
		this.emitter = new n4_effects_particles_NParticleEmitter(200);
		this.add(this.emitter);
		var pbt = new n4e_ui_NEText(20,n4_NGame.height * 0.65,"use the arrow keys or ijkl to move. use f or m to shoot.",26);
		pbt.screenCenter(n4_util_NAxes.X);
		this.add(pbt);
		if(this["transient"]) {
			n4_NGame.timers.setTimer(1400,function() {
				n4_NGame.switchState(_gthis.nextState);
			});
		} else {
			var howToStartText = new n4e_ui_NEText(0,n4_NGame.height * 0.75,"press F to return",20);
	...
	,update: function(dt) {
		...
		while(_g < 12) {
			++_g;
			this.emitter.emitSquare(n4_NGame.width / 2,n4_NGame.height / 2,Math.random() * 6 + 3 | 0,n4_effects_particles_NParticleEmitter.velocitySpread(120),n4_util_NColorUtil.randCol(0.9,0.9,0.2,0.1),2.2);
		}
		if(!this["transient"] && n4_NG.keys.justPressed(["F"])) {
			n4_NGame.switchState(Registry.MS);
		}
		n4_NState.prototype.update.call(this,dt);
	}
	,__class__: states_HelpState
});
...
states_IntroState.prototype = $extend(n4_NState.prototype,{
	create: function() {
		if(Registry.mainFont == null) {
			Registry.mainFont = kha_Assets.fonts.champagneLimousines;
		}
		if(n4e_ui_NEText.defaultFont == null) {
			n4e_ui_NEText.defaultFont = Registry.mainFont;
		}
		this.add(new n4e_ui_NEText(20,20,"SuperBoats",35,-1));
...
states_MenuState.prototype = $extend(n4_NState.prototype,{
	...
	,create: function() {
		...
		this.emitter = new n4_effects_particles_NParticleEmitter(200);
		this.add(this.emitter);
		this.effectEmitter = new n4_effects_particles_NParticleEmitter(120);
		this.add(this.effectEmitter);
		this.dummyBoats = new n4_group_NTypedGroup();
		var _g1 = 0;
		var _g = (Math.random() * 6 | 0) + 1;
		while(_g1 < _g) {
			++_g1;
			var dummyBoat = new sprites_DummyBoat(Math.random() * n4_NGame.width,Math.random() * n4_NGame.height);
		...
		var howToStartText = new n4e_ui_NEText(0,n4_NGame.height * 0.5,"Press F to play, H for help",30);
		howToStartText.screenCenter(n4_util_NAxes.X);
		this.add(howToStartText);
		this.lastLevel = Registry.levelNum;
	...
	,update: function(dt) {
		...
		if(n4_NG.keys.justPressed(["F"])) {
			this.startGame();
		}
		if(n4_NG.keys.justPressed(["H"])) {
			n4_NGame.switchState(new states_HelpState(false));
		}
		if(Registry.levelNum != this.lastLevel) {
			this.lastLevel = Registry.levelNum;
			this.updateLevelDisplay();
		}
		var _g = 0;
		while(_g < 12) {
			++_g;
			this.effectEmitter.emitSquare(n4_NGame.width / 2,n4_NGame.height / 3,Math.random() * 7 + 3 | 0,n4_effects_particles_NParticleEmitter.velocitySpread(420),n4_util_NColorUtil.randCol(0.2,0.6,0.8,0.2),2.2);
		}
	...
	,updateLevelDisplay: function() {
		if(this.levelText != null) {
			HxOverrides.remove(this.members,this.levelText);
		}
		this.levelText = new n4e_ui_NEText(0,n4_NGame.height * 0.67,"level " + Registry.levelNum,20);
		this.levelText.screenCenter(n4_util_NAxes.X);
		this.add(this.levelText);
	}
	,startGame: function() {
		if(Registry.PS == null) {
			var shouldShowHelp = !Registry.shownHelp;
			var target = new states_PlayState();
			if(shouldShowHelp) {
				Registry.shownHelp = true;
				n4_NGame.switchState(new states_HelpState(true,target));
			} else {
				n4_NGame.switchState(target);
			}
		} else {
			n4_NGame.switchState(Registry.PS);
		}
	}
...
states_PlayState.prototype = $extend(n4_NState.prototype,{
	...
	,create: function() {
		...
		this.add(this.explosionEmitter);
		this.smokeEffectEmitter = new n4_effects_particles_NParticleEmitter(140);
		this.add(this.smokeEffectEmitter);
		this.helpText = new n4e_ui_NEText(0,n4_NGame.height / 3,"",40);
		this.add(this.helpText);
		n4_NState.prototype.create.call(this);
	}
	,update: function(dt) {
		n4_NG.overlap(this.allies,this.projectiles,$bind(this,this.allyHitProjectile));
		n4_NG.overlap(this.warships,this.playerProjectiles,$bind(this,this.warshipHitProjectile));
		if(n4_NG.keys.justPressed(["ESC"])) {
			n4_NGame.switchState(Registry.MS);
		}
		n4_NState.prototype.update.call(this,dt);
	...
	,checkGameStatus: function() {
		Registry.lastMsDamage = this.mothership.get_damage();
		if(!this.player.exists) {
			n4_NGame.timers.setTimer(500,function() {
				Registry.PS = null;
				n4_NGame.switchState(new states_GameOverState());
			});
		}
		if(!this.mothership.exists) {
			n4_NGame.timers.setTimer(500,function() {
				Registry.PS = null;
				n4_NGame.switchState(new states_YouWonState());
			});
		}
	}
...
var states_YouWonState = function() {
	n4_NState.call(this);
};
$hxClasses["states.YouWonState"] = states_YouWonState;
states_YouWonState.__name__ = true;
states_YouWonState.__super__ = n4_NState;
states_YouWonState.prototype = $extend(n4_NState.prototype,{
	emitter: null
	,create: function() {
		this.add(new n4e_ui_NEText(20,20,"SuperBoats",35,-1));
		this.emitter = new n4_effects_particles_NParticleEmitter(200);
...
n4_NG.worldBounds = new n4_math_NRect();
n4_NG.worldDivisions = 6;
n4_NG.cameras = [];
...
n4_NGame.stabilizeUpdateInterval = false;
...
n4_math_NVector.EPSILON_SQUARED = 9.99999999999999841e-15;
